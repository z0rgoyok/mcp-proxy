...
SCOPE_DIR="${1:-src/main/kotlin/dev/mcp/proxy}"
...
qodana scan \
  ...
  --results-dir "$ROOT_DIR/build/qodana-results-fast" \
  --report-dir "$ROOT_DIR/build/qodana-report-fast" \
  --only-directory "$SCOPE_DIR" \
  --print-problems \
