...
class MockProxyServerTest {
    ...
    fun `mock rule returns fixture and writes journal and readable log events`() = runTest {
        ...
        try {
            val request = HttpRequest.newBuilder(URI.create("http://127.0.0.1:$port/health"))
                .GET()
                .header("X-city", "3")
                .header("X-Pfm", "1111")
                .header("X-Delivery-Type", "store:1111")
            ...
            assertContains(journal, """"requestHeaders":{""")
            assertContains(journal, """"X-city":["3"]""")
            assertContains(journal, """"X-Pfm":["1111"]""")
            assertContains(journal, """"X-Delivery-Type":["store:1111"]""")
            assertContains(journal, """"Authorization":["[REDACTED]"]""")
            assertFalse(journal.contains("Bearer secret"))
            val adminJournal = get("http://127.0.0.1:$port/admin/api/journal?limit=1").body()
            assertContains(adminJournal, """"requestHeaders":{""")
            assertContains(adminJournal, """"X-city":["3"]""")
            assertContains(adminJournal, """"X-Pfm":["1111"]""")
            assertContains(adminJournal, """"X-Delivery-Type":["store:1111"]""")
            assertContains(adminJournal, """"Authorization":["[REDACTED]"]""")
            assertFalse(adminJournal.contains("Bearer secret"))
    ...
    fun `mitm proxy decrypts https request and returns fixture`() = runTest {
        ...
        try {
            ...
            val journal = stateDirectory.resolve("journal/events.jsonl").toFile().readText()
            val normalizedJournal = journal.lowercase()
            assertContains(journal, """"mode":"mock"""")
            assertContains(normalizedJournal, """"x-pfm":["4103"]""")
            assertContains(normalizedJournal, """"x-api-key":["[redacted]"]""")
            assertFalse(journal.contains("secret"))
            val requestLog = assertIs<ProxyLogEvent.RequestHandled>(eventLogger.events[1])
            assertEquals("GET", requestLog.method)
            assertEquals("/health", requestLog.path)
            assertEquals("mock", requestLog.mode)
